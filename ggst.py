...
class GGSTWebScraper:
    ...
    async def search_player(self, search: str) -> list[bs4.element.Tag]:
        ...
        async with self.session.get(f"http://ratingupdate.info/?name={search}") as resp:
            ...
            res: list[bs4.element.Tag] = soup.find(
                "div", class_="table-container"
            ).find_all("td")
            for e in res:
                if e.find("a"):
                    title = e.find("a")["title"]
                    if title == search:
                        results.append(e)
        if results == []:
            raise Exception(
                f"Player {search} not found. Be sure to type your username correctly, accounting for case and other factors."
            )
        return results
        ...
        for e in data:
            if e.find("a"):
                try:
                    ...
                    raise Exception(
                        "Character not found, ensure you are typing it exactly as it is found on the ratingupdate website."
                    )
    ...
    def get_char_matchup_stats(
        self, data: list[bs4.element.Tag], char: str
    ) -> MatchupStats:
        for e in data:
            if e == data[0]:
                continue
    ...
    async def get_elo(self, path: str) -> EloStats:
        async with self.session.get(f"http://ratingupdate.info{path}") as resp:
            ...
            string = " ".join(res.replace("\n", "").replace("#", " #").split()).split()
            return {
                "character": string[0],
                "elo": int(string[2]),
                "games": int(string[4][1:]),
                "offset": int(string[3][1:]),
                "position": int(string[6][1:]) if len(string) >= 7 else None,
            }
